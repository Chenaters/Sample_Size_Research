...
### Bonferroni control for Type I error rate in multiple-hypothesis testing
  prediN=200; #number of candidate predictors to be tested simultaneously
  alphaLevel = 0.05/prediN; 
  
  subjN = 300; #Restriction for the number of subjects
  breaksN = seq(50, subjN, by=50); #Numbers of subjects at which power values are obtained
...
###Generate model
  if (respoType == "binaryRespo") {
    ...
    print(mean(simY)); #print the frequency of binary response.
    ...
    simY = doSim(model); #simulate response, each run is a random generation (without setting seeds)
    #print(c(mean(simY), sd(simY))); #print the mean and sd of continuous response.
    print(c(mean(simY[which(covars$time==0)]), sd(simY[which(covars$time==0)]))); #print the baseline mean and sd of continuous response
  }
...
###Power analysis. The null model is that predictor has no effects: y~time. 
  ...
  simY = doSim(model); #simulate response, each run is a random generation (without setting seeds)
  if (respoType == "binaryRespo") {
    propY = round(mean(simY), 1); #proportion of Y (positive response)
    print(propY);
    fileName = paste(fileName, "_propYb_", propY, "_prediN_", prediN,  ".png", sep="");
  }else {
    meanYb = round(mean(simY[which(covars$time==0)]), 1); sdYb = round(sd(simY[which(covars$time==0)]), 1); #mean and sd of Y at baseline 
    print (c(meanYb, sdYb));
