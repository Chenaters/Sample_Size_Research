...
N = seq(50, 2000, by=10); #Sample sizes
...
for (f2 in F2){
  ...
  for (p2 in P2){
    ...
    out = effi.n(p1=p1, p2=p2, evr=1/(f2+1), effi=0.8);
    print(c(f2, p2, as.vector(out))); #Efficient sample size. 
  }
  ...
  trendCurves(dat=pPMSErs, trendValue=N,
              ...
              fileName=paste("Cohenf2_p1_", p1, "_f2_", f2, "_nstar", ".png", sep=""),
              xlab = "sample size", ylab="pPMSEr", main=paste0("f2 ", f2),ylim=c(0, 0.5));
}
